...
**1.** Воспользуемся нашей формулой. В четвертой компоненте происходит два отражения, поэтому подставляем $k=2$, и получаем:

\begin{equation}
\alpha^\* = \mathrm{arctg}\sqrt{\dfrac{9-n^2}{n^2-1}}
\end{equation}

Подставляем показатели преломления для красного и синего цвета и получаем:

\begin{equation}
\alpha^\*_{\color{red}\text{кр}} = 71{,}91^\circ 
\qquad\qquad
\alpha^\*_{\color{blue}\text{син}} = 71{,}52^\circ 
\end{equation}

\begin{equation}
\theta^\*_{\color{red}\text{кр}} = 230{,}4^\circ 
\qquad\qquad
\theta^\*_{\color{blue}\text{син}} = 233{,}5^\circ 
\end{equation}

Обратите внимание, углы поворота получились больше $180^\circ$, но все-таки ближе к $180^\circ$, чем к $360^\circ$. Это означает, что радуга будет наблюдаться в полусфере противоположенной Солнцу. Угол $\theta^\*_{\color{red}\text{кр}}$ ближе к $180^\circ$, чем $\theta^\*_{\color{blue}\text{син}}$. Это означает, что красная полоса радуги находится внутри, а синяя и фиолетовая снаружи. И наконец, полученные нами углы для второй радуги $\theta^\*\approx 232^\circ$ немного дальше от $180^\circ$, чем углы для первой радуги $\theta^\*\approx 138^\circ$. И правда
